...
(function(window, undefined) {
  ...
  function renderAttachment(root,comment,callback){
	...
	if($image.length){
		...
		getAttachmentFilePath(comment.uuid,comment.fileName,this,function(currentImage,source){
    ...
    if (comments && comments.length>0) {
      ...
      for(c=0, len=comments.length; c<len; c+=1) {
        ...
        if (comment.parentCode) {
          ...
          renderAttachment($root.find("input#code[value='" + comment.uuid + "']").parent()[0],comment);
...
// }
  ...
  function loadDataRemote(refresh) {
	if(jimUtil.isFileProtocol() || $("body").hasClass("offline"))
		return;
